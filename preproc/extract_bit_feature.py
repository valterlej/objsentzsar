"""
Code from Elaboratire Reheasal
...
class VideoDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, idx):
    ...
    try:
      ...
      vid = load_video(os.path.join(self.video_dir, videoname))
      nframes = vid.shape[0]
...
def gather_data(args, video_list):
  ...
  json.dump(outs, open(os.path.join(args.output_dir, '..', '%s_top1000_preds.json'%(args.model.lower())), 'w'))
