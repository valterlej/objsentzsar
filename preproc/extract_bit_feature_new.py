...
class VideoDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, idx):
    ...
    try:
      ...
      # criar o arquivo temporario a x fps
      ...
      if nframes <= self.num_frames_per_video:
        ...
        #idxs = np.linspace(0, nframes-1, self.num_frames_per_video) ### extract exactly num_frames_per_video
        idxs = np.linspace(0, nframes-1, nframes // self.num_frames_per_video) #### um frame a cada num_frames_per_video
...
def gather_data(args, video_list):
  ...
  #json.dump(outs, open(os.path.join(args.output_dir, '..', '%s_top20_preds.json'%(args.model.lower())), 'w'))
  json.dump(outs, open(os.path.join(args.output_dir, '..', '%s_top30_preds.json'%(args.model.lower())), 'w'))
...
# python extract_bit_feature_new.py --model BiT-M-R152x2 --model_dir data/models/ --video_dir /home/valter/datasets/activitynetcaptions/ --output_dir /home/valter/datasets/activitynetcaptions_bit/ --output_feat_dir /home/valter/datasets/activitynetcaptions_features_bit/ --num_workers 2 --num_frames_per_video 25